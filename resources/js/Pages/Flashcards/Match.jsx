...
const Match = ({
    ...
    const addIncorrectTranslations = () => {
        const translations = cards.filter(
            (c) => c.id === cards[firstCardIndex].id,
        );
                ...
                {!isStarted && !isEnd && (
                    ...
                    >
                        ...
                        <span className="text-gray-700 text-xl md:max-w-[35%] text-center border-b-4 border-gray-300 pb-2">
                            Match all the terms with their definitions as quick
                            as possible. Try avoiding incorrect matches. They
                            add extra time!
                        </span>
                        <div
                ...
                {isStarted && (
                    <>
                        <div className="mb-4 flex flex-col gap-2">
                            ...
                            >
                                <i className="fa-solid fa-hourglass text-md transform animate-hourglass"></i>
                                {seconds}
                            </div>
                        </div>
                            ...
                            {cards.map((card, index) => {
                                if (currentSetPage === card.page) {
                                    return (
                                        <button
                                            ref={(element) => {
                                                cardRefs.current[card.refId] =
                                                    element;
                                            }}
                                            disabled={
                                                disabledCards.includes(
                                                    card.id,
                                                ) ||
                                                isSecondCard ||
                                                card.isDisabled
