    ...
    const onSubmit = (data) => {
        if (!Object.keys(data).some((key) => data[key] === true)) {
            ...
            const formData = {
                ...
                testTypes: Object.keys(data).filter(
                    (key) => data[key] === true,
                ),
            ...
            makeTest(
                shuffledTranslations.splice(0, data.questionsLength),
                formData,
            );
            setIsStarted(true);
        }
                            ...
                            data:
                                randomComponentName !== "TrueOrFalseAnswer"
                                    ...
                                    : {
                                          ...trueOrFalseData[index],
                                          term: {
                                              word: trueOrFalseData[index]
                                                  .definition.word,
                                          },
                                          definition: {
                                              word: trueOrFalseData[index].term
                                                  .word,
                                          },
                                      },
                            isFilled: false,
    ...
    return (
        <Container>
            <form
                ...
                className="flex gap-8 flex-col relative polygon-start opacity-0 translate-y-12 text-md"
                ref={(element) => {
            ...
            >
                ...
                <div
                    className="relative flex flex-col gap-2 bg-white py-4 space-y-2 rounded-md polygon-start opacity-0 translate-y-12 before:absolute before:w-1 before:h-full before:bg-indigo-400 before:top-0 before:left-3 pl-6 pr-4 text-gray-700"
                    ref={(element) => {
                ...
                <div
                    className={
                        "flex gap-4 flex-col polygon-start opacity-0 translate-y-12 bg-white py-4 space-y-2 rounded-md before:absolute before:w-1 before:h-full before:bg-indigo-400 before:top-0 before:left-3 pl-6 pr-4 text-gray-700"
                    }
                    ref={(element) => {
